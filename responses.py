...
def predict_class(sentence):
    bag_of_words = bow(sentence, 'false')
    resp = model.predict(np.array([bag_of_words]))[0]
    ERROR_THRESHOLD = 0.25
    results = [[i, r] for i, r in enumerate(resp) if r > ERROR_THRESHOLD]
...
def sent_response(msg):
    inns = predict_class(msg)
    print(inns)
    flag = (json.dumps(inns).__contains__('apply_leave'))
    if msg != "bye":
        # if(flag):
        response = get_response(inns, intents)
        # print(intents)
        return response
    else:
